...
public:
  ...
  void Authenticate(FlightSqlConnection &connection,
    ...
    // call_options may have already been populated with data from the connection string or DSN.
    // Ensure auth-generated headers are placed at the front of the header list.
  ...
  std::string password_;
    ...
    public:
        ...
        void Authenticate(FlightSqlConnection &connection, FlightCallOptions &call_options) override {
            // add the token to the front of the headers. For consistency auth headers should be at the front.
